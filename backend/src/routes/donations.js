...
router.get('/', async (req, res) => {
  try {
    ...
    const donations = await Donation
      .find(filter)
      .sort({ createdAt: -1 })
      .populate('donor', 'nombre apellido name ubicacion provincia location');
...
router.post('/', auth, upload.array('images', 3), async (req, res) => {
  try {
    ...
    // Extraer el donor del token JWT del usuario autenticado
    const donor = await User.findOne({ id: req.user.id });
    if (!donor) {
      console.log('❌ Usuario no encontrado');
      return res.status(404).json({ error: 'Usuario no encontrado' });
    }
    donationData.donor = donor._id;
