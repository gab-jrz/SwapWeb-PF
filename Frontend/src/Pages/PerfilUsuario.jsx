...
const PerfilUsuario = () => {
  ...
  const agruparMensajes = (mensajes, userId) => {
    ...
    mensajes.forEach(msg => {
      ...
      // Crear chatId basado en si es donación o producto
      let chatId;
      if (msg.donacionId) {
        // Para donaciones, usar donacionId + otroUsuarioId
        chatId = [msg.donacionId, otroUsuarioId].sort().join('_donacion_');
      } else {
        // Para productos (intercambios), usar productoId + otroUsuarioId
        chatId = [msg.productoId, otroUsuarioId].sort().join('_');
      }
      ...
      <div className="perfil-usuario-content">
        ...
        <div className="perfil-tab-content">
                  ...
                  {favoritos.map((producto) => (
                    <div key={producto.id || producto._id} className="favorito-card-premium">
                      <div className="favorito-img-wrap">
                        <img
                          src={getProductImageUrl(producto.images || producto.image)}
                          alt={producto.title}
          ...
          {activeTab === 'donaciones' && (
            <div className="mis-donaciones" ref={donacionesSectionRef}>
              <h2>Mis Donaciones</h2>
              <button className="btn-publicar" onClick={() => navigate("/donaciones/publicar")} style={{ marginBottom: '1rem' }}>
                + Nueva Donación
              </button>
              {loadingDonaciones ? (
                <div className="loading-spinner">Cargando donaciones...</div>
              ) : donaciones.length === 0 ? (
                <p>No has publicado ninguna donación aún.</p>
              ) : (
                <div className="articulos-grid">
                  {donaciones.map((donacion) => (
                    <div key={donacion._id || donacion.id} className="donacion-card">
                      {/* Imagen de la donación */}
                      <div className="donacion-image-container">
                        {donacion.images && donacion.images.length > 0 ? (
                          <img 
                            src={getDonationImageUrl(donacion.images[0])} 
                            alt={donacion.title || donacion.itemName}
                        ...
                        <div className="donacion-image-placeholder" style={{display: donacion.images && donacion.images.length > 0 ? 'none' : 'flex'}}>
                      ...
                      <div className="donacion-content">
                        <div className="donacion-header">
                          <span className="donacion-categoria">{donacion.category || donacion.categoria}</span>
                          <span className={`donacion-status ${donacion.status || 'available'}`}>
                            {donacion.status === 'completed' ? 'Donado' : 
                             donacion.status === 'pending' ? 'Pendiente' : 'Disponible'}
                          </span>
                        </div>
                        <h4 className="donacion-title">{donacion.title || donacion.itemName}</h4>
                        <p className="donacion-desc">{donacion.description}</p>
                        <div className="donacion-meta">
                          <div className="donacion-fecha">
                            {new Date(donacion.createdAt).toLocaleDateString()}
                          </div>
                          <div className="donacion-ubicacion">
                            {donacion.location || 'Sin especificar'}
                          </div>
                        </div>
                        <div className="donacion-actions">
                          <button 
                            className="btn-ver"
                            onClick={() => navigate(`/donation/${donacion._id || donacion.id}`)}
                          >
                            Ver Detalles
                          </button>
                          <button 
                            className="btn-editar"
                            onClick={() => navigate(`/donation-edit/${donacion._id || donacion.id}`)}
                          >
                            Editar
                          </button>
                        </div>
