...
const DonationCreateNew = () => {
  ...
  const handleFileSelect = async (files) => {
    ...
    if (previewImages.length >= 3) {
      // Desplazar el área de upload para dar contexto
      uploadAreaRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      showNotification('Máximo 3 imágenes permitidas', 'error');
      return;
    }

    // Si la cantidad seleccionada excede el límite restante, notificar y recortar
    const remaining = 3 - previewImages.length;
    const toAdd = Math.min(files.length, remaining);
    if (files.length > remaining) {
      const ignored = files.length - toAdd;
      uploadAreaRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      showNotification(`Se agregaron ${toAdd} imagen(es). ${ignored} excede(n) el máximo de 3 y fueron ignoradas.`, 'error');
    }
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.images || formData.images.length === 0) {
      // Llevar el foco visual a la galería
      uploadAreaRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      showNotification('Agrega al menos 1 foto de la donación', 'error');
      return;
    }
    // No permitir enviar si excede el límite de imágenes
  ...
  // Cuando aparece una notificación de error, asegurar que sea visible
  useEffect(() => {
    if (notification.show && notification.type === 'error') {
      // Esperar al render del nodo antes de desplazar
      const t = setTimeout(() => {
